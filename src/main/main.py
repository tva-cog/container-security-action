    ...
    for finding in findings:
        if "nvdFinding" in finding:
            if "cve" in finding["nvdFinding"] and "cvss_score" in finding["nvdFinding"]:
                cve_info[finding["nvdFinding"]["cve"]] = finding["nvdFinding"][
                    "cvss_score"
                ]
...
def get_report(url, access_key, secret_key):
    ...
    while (
        ...
        and response_dict["status"] == "error"
        and response_dict["message"] == "report_not_ready"
    ):
    ...
    if (
        "findings" not in response_dict
        or "risk_score" not in response_dict
        or "malware" not in response_dict
    ):
...
def push_docker_image(access_key, secret_key, registry, repository, image, tag):
    ...
    try:
        ...
        login_response = client.login(
            username=access_key, password=secret_key, registry=registry
        )
        ...
        tagging_response = client_image.tag(
            f"registry.cloud.tenable.com/{image}", tag=f"{tag}"
        )
...
def scan(repository, tag):
    ...
    check_thresholds = (
        True if str(os.environ["INPUT_CHECK_THRESHOLDS"]) == "true" else False
    )
    ...
    logger.info(
        f"::set-output name=number_of_malware_findings::{number_of_malware_findings}"
    )
